...
env | grep -E '^TCP_TUNNEL_PORT_([0-9]+)=' | sed -E 's/^TCP_TUNNEL_PORT_([0-9]+)=(.+):([0-9]+)/\1 \2 \3/' | while read -r local_port remote_host remote_port ; do
...
env | grep -E '^SSH_TUNNEL_PORT_([0-9]+)=' | sed -E 's/^SSH_TUNNEL_PORT_([0-9]+)=(.+):(.+)@(.+):([0-9]+)/\1 \2 \3 \4 \5/' | while read -r local_port user credentials remote_host remote_port ; do
